...
public class PlayerSpawner : MonoBehaviour
{
	...
	public GameObject Spawn (GameObject player)
	{
		...
		if (Network.isClient || Network.isServer) {
			...
			return (GameObject)Network.Instantiate (player, spawnPosition, Quaternion.identity, 0);
		} else {
			Debug.Log ("Spawn " + player.name);
			return (GameObject)GameObject.Instantiate (player, spawnPosition, Quaternion.identity);
		}
	}
